...
Public Class frmcount
    ...
    Public i As Integer
    Private Sub frmcount_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        cmd.Connection = cn
        loadgrid1()
        loadgrid2()
        loadgrid3()
    ...
    Sub loadgrid1()
        ...
        'rs.Open(sql, conn)
        'Do While Not rs.EOF
        cmd.CommandText = "select * from tbl_vote where (post = N'PRESIDENT')"
    ...
    Sub loadgrid2()
        ...
        'rs.Open(sql, conn)
        'Do While Not rs.EOF
        cmd.CommandText = "select * from tbl_vote where (post = 'SECRETARY')"
        dr = cmd.ExecuteReader()
        If dr.HasRows Then
            While (dr.Read())
    ...
    Sub loadgrid3()
        ...
        'rs.Open(sql, conn)
        'Do While Not rs.EOF
        cmd.CommandText = "select * from tbl_vote where post = 'JOINT SECRETARY' "
        dr = cmd.ExecuteReader()
        If dr.HasRows Then
            While (dr.Read())
